...
exports.getAllTransactionsByUser = async (req, res) => {
  ...
  if (!tableInfo) {
    return res
      .status(400)
      .json({ error: true, message: "Invalid transaction type" });
  }
...
exports.getTransactionById = async (req, res) => {
  ...
  try {
    ...
    if (!laporanData) {
      return res
        .status(404)
        .json({ error: true, message: "Transaction not found in report" });
    }
    ...
    if (!transactionData) {
      return res
        .status(404)
        .json({ error: true, message: "Transaction data not found" });
    }
...
exports.createTransaction = async (req, res) => {
  ...
  try {
    ...
    const laporanData = {
      ...
      tanggal: currentDate, // Gunakan currentDate
      ...(type === "pengeluaran"
        ? { idTransaksiPengeluaran: Transaksi.idTransaksiPengeluaran }
...
exports.exportToPDF = async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(15).text("Pemasukan", 50, 100);
    ...
    pemasukanTable.rows.push(["", "Total Pemasukan", totalPemasukan, "", ""]);
...
exports.getTotalTransactions = async (req, res) => {
  ...
  try {
    ...
    const total = transactions.reduce(
      (acc, transaction) => acc + transaction.jumlah,
      0
    );
